 ...
 *     spacing_y,
 *     offset_y,
 *     offset_x, // function for getting actual left padding
 * }
 */

const flowy = function (options) {
    // TODO: needs to be validated
    const canvas_div = options.canvas;
    let grab = options.grab;
    let release = options.release;
    let snapping = options.snapping;
    let spacing_x = options.spacing_x;
    let spacing_y = options.spacing_y;
    const offset_y = options.offset_y;
    let blocks = [];
    let blockstemp = [];
    ...
    flowy.load = function () {
        ...
        // Delete block by id and all his child blocks
        let preparedBlocks = [];
        const indicator = document.querySelector(".indicator");
        flowy.deleteBlockById = function (id, cb) {
            if (typeof id !== "number") {
                throw "Id should be a number";
            }
            preparedBlocks = blocks.reduce((ac, block) => {
                ac[block.id] = block;
                return ac;
            }, {});

            const blocksIdToDelete = [];
            deleteGraph(id, (blockId) => {
                document.querySelector(".blockid[value='" + blockId + "']").parentNode.remove();
                document.querySelector(".arrowid[value='" + blockId + "']").parentNode.remove();
                delete preparedBlocks[blockId];
                blocksIdToDelete.push(blockId);
            });

            blocks = blocks.length > 0 ? Object.values(preparedBlocks) : [];
            const allBlocks = document.querySelectorAll('.block');
            if (allBlocks && !document.querySelector(".indicator")) {
                allBlocks[allBlocks.length - 1].appendChild(indicator);
            }
            cb(blocks);
        }

        flowy.getBlock = (id) => {
            return blocks.filter(b => b.id === Number(id))[0];
        };

        function deleteGraph(id, cb) {
            if (id === 0) {
                flowy.deleteBlocks();
                return;
            }
        ...
        flowy.beginDrag = function (event) {
            ...
            if (event.which != 3 && event.target.closest(".create-flowy")) {
                ...
                active = true;
                dragx = mouse_x - (event.target.closest(".create-flowy").offsetLeft) - options.offset_x();
                dragy = mouse_y - (event.target.closest(".create-flowy").offsetTop) - offset_y;
                ...
                if (parseInt(drag.querySelector(".blockid").value) === 0 && rearrange) {
                    ...
                    for (let w = 0; w < blockstemp.length; w++) {
                        if (blockstemp[w].id != parseInt(drag.querySelector(".blockid").value)) {
                            ...
                            arrowParent.style.left = (arrowParent.getBoundingClientRect().left + window.scrollX) - (canvas_div.getBoundingClientRect().left + window.scrollX) + canvas_div.scrollLeft + 'px';
                    ...
                    active = false;
                    const left = (drag.getBoundingClientRect().left + window.scrollX) - (canvas_div.getBoundingClientRect().left + window.scrollX) + canvas_div.scrollLeft;
                    const top = (drag.getBoundingClientRect().top + window.scrollY) - (canvas_div.getBoundingClientRect().top + window.scrollY) + canvas_div.scrollTop;
                    drag.style.top = top + "px";
                    drag.style.left = left + "px";
                    canvas_div.appendChild(drag);
                    const x = (drag.getBoundingClientRect().left + window.scrollX) + (parseInt(window.getComputedStyle(drag).width) / 2) + canvas_div.scrollLeft;
                    const y = (drag.getBoundingClientRect().top + window.scrollY) + (parseInt(window.getComputedStyle(drag).height) / 2) + canvas_div.scrollTop;
                    blocks.push({
                        parent: -1,
                        childwidth: 0,
                        id: parseInt(drag.querySelector(".blockid").value),
                        type: parseInt(drag.querySelector(".blockelemtype").value),
                        x,
                        y,
        ...
        function addArrow(drag, i, blocko) {
        ...
        function snap(drag, i, blocko) {
            ...
            const left = blocks.filter(id => id.id == blocko[i])[0].x - (totalwidth / 2) + totalremove - (canvas_div.getBoundingClientRect().left + window.scrollX) + canvas_div.scrollLeft;
            const top = blocks.filter(id => id.id == blocko[i])[0].y + (blocks.filter(id => id.id == blocko[i])[0].height / 2) + paddingy - (canvas_div.getBoundingClientRect().top + window.scrollY);
            drag.style.left = left + "px";
            drag.style.top = top + "px";
            if (rearrange) {
        ...
        flowy.addArrow = addArrow;

        flowy.setBlocks = function (data) {
            blocks.push(data);
        };
                ...
                while (!flag) {
                    for (var i = 0; i < layer.length; i++) {
                        if (layer[i] != blockid) {
                            ...
                            const dragPosition = drag.getBoundingClientRect();
                            const parentPosition = blockParent.getBoundingClientRect();
                            const arrowParentPosition = arrowParent.getBoundingClientRect();
                            const offsetY = window.pageYOffset;
                            const offsetX = window.pageXOffset;
                            const perentTop = (parentPosition.top + offsetY) - (dragPosition.top + offsetY);
                            blockParent.style.left = (parentPosition.left + offsetX) - (dragPosition.left + offsetX) + 'px';
                            blockParent.style.top = (parentPosition.top + offsetY) - (dragPosition.top + offsetY) + 'px';
                            arrowParent.style.left = (arrowParentPosition.left + offsetX) - (dragPosition.left + offsetX) + 'px';
                            arrowParent.style.top = (arrowParentPosition.top + offsetY) - (dragPosition.top + offsetY) + 'px';
        ...
        function isCanBeSnaped(blockParent, block) {
            return true;
        }
